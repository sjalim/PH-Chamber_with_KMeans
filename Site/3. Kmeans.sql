		 ...
		 if area_type = 0 then -- local
			...
			dbms_output.put_line('at if');
				...
				if select_city = 'Barisal' then
						for i in (select sulfur_dixd_lvl, nitro_oxid_lvl,ph_lvl from TestData@server1 where city = select_city)
					loop
			...
			for i in 
			(
			select sulfur_dixd_lvl, nitro_oxid_lvl,ph_lvl from TestData@server1 where city = select_city
			union 
	...
	begin
		...
		if areaType = 0 then -- local 
			...
			if select_city = 'Barisal' then
				...
				for d in (select train_id, sulfur_dixd_lvl, nitro_oxid_lvl, ph_lvl from train@server1 where city = city ) loop
				...
				for d in (select train_id, sulfur_dixd_lvl, nitro_oxid_lvl, ph_lvl from train where city = city ) 
				loop
					dt := datasetObj(d.train_id, d.sulfur_dixd_lvl, d.nitro_oxid_lvl, d.ph_lvl);
					dataset.extend;
					dataset(dataset.last) := dt;
			...
			for d in (
			select train_id, sulfur_dixd_lvl, nitro_oxid_lvl, ph_lvl from train@server1 
			union 
...
begin
	...
	dbms_output.put_line('alim');
	kmeans.getTempTrainData(inpt,0);
	--kmeans.display;
